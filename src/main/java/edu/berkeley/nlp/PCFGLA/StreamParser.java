 ...
 *
 * Taken from release-1.0 of the berkeleyparser project. Modified to support io
 * with arbitrary streams.
 *
 * @author Slav Petrov, Mark Farrell
 */
public class StreamParser  {
	...
	public static void parse(InputStream parserIn, PrintStream parserOut, String[] args) throws Exception {
      ...
      Binarization bin = null;
			for (int nGr = 0; nGr < opts.nGrammars; nGr++){
				...
				if (pData==null) {
					parserOut.println("Failed to load grammar from file"+inFileName+".");
					throw new Exception();
				}
      ...
      parser.binarization = bin;
		...
		try{
			BufferedReader inputData = (opts.inputFile==null) ? new BufferedReader(new InputStreamReader(parserIn)) : new BufferedReader(new InputStreamReader(new FileInputStream(opts.inputFile), "UTF-8"));
			PrintWriter outputData = (opts.outputFile==null) ? new PrintWriter(new OutputStreamWriter(parserOut)) : new PrintWriter(new OutputStreamWriter(new FileOutputStream(opts.outputFile), "UTF-8"), true);
	...
	private static void outputTrees(List<Tree<String>> parseTrees, PrintWriter outputData, 
			CoarseToFineMaxRuleParser parser, edu.berkeley.nlp.PCFGLA.StreamParser.Options opts, 
